...
class LoginTest(unittest.TestCase):
    ...
    @allure.severity(allure.severity_level.BLOCKER)
    def test_x_AutoClean(self):

        window.scrollTo(0, 200)
        self.driver.execute_script("window.scrollTo(0, window.scrollY + 400)")
        sleep(4)
        # parent_h = self.driver.current_window_handle
        # handles = self.driver.window_handles
        # self.driver.remove(parent_h)
        # print(self.driver.getTitle())
        Auto_clean = self.driver.find_element_by_xpath('//*[@id="root"]/div/div/div/div/div/div/div/div[2]/div[2]/div/div[2]/div/div[1]/div/div/div/div/div/div[1]/div/div[2]/div/div[2]/div[2]')
        Auto_clean.click()
        sleep(4)
        # self.driver.switch_to.window(window_handle)
        # self.driver.switch_to_window(handles.pop())
        # self.driver.switch_to_alert(self)
        # start_btn = self.driver.find_element_by_xpath('/html/body/div[4]/div[3]/div/div/div[2]/div[3]/button/span[2]')
        # start_btn = self.driver.find_element_by_xpath("//span[contains(.,'Start Auto Clean')]")
        start_btn = self.driver.find_element(By.XPATH, "//span[contains(.,'Start Auto Clean')]")
        # "//span[@id='MuiButton-label']"
        start_btn.click()
        # #
        sleep(4)
        print('Auto-Pilot mode has been turned on successfully')
        self.driver.close()
